...
var textNoNew = window.PAGE_NOTIFICATIONS_NO_NEW || "У вас нет новых уведомлений";
var textNewCount = window.PAGE_NOTIFICATIONS_NEW_COUNT || "У вас <b>{count}</b> новых уведомлений";
...
function updateCountText(count) {
  ...
  if (count === 0) {
    ...
    countElement.innerHTML = window.PAGE_NOTIFICATIONS_NO_NEW || "У вас нет новых уведомлений";
  } else {
    // choose plural form based on count
    var pluralText = ngettext("PAGE_NOTIFICATIONS_COUNT_SINGLE", "PAGE_NOTIFICATIONS_COUNT_PLURAL", count);
    countElement.innerHTML = interpolate(pluralText, {
      counter: count
    }, true);
  }
}
...
function renderNotification(data, isHistory) {
  var isNew = data.ViewedAt === null || data.ViewedAt === undefined || data.ViewedAt === 0;
  var meta = data.Meta || {};
  ...
  var iconPath = iconSrc.indexOf("/") !== -1 ? iconSrc : "/staticfiles/img/ntficons/" + iconSrc;
  var notif_title = window.notifs[data.Title] || data.Title;
  var notif_content = window.notifs[data.Content] || data.Content;
  var inner = '<div class="notify_ic"><img src="' + iconPath + '" alt="icon"></div>' + '<div class="notify_body">' + '<div class="notify_title">' + notif_title + "</div>" + '<div class="notify_desc">' + notif_content + "</div>";
  if (meta.action_url) {
    var actionText = meta.action_text || "Перейти »";
    inner += '<a href="' + meta.action_url + '" class="notify_action">' + actionText + "</a>";
  }
  ...
  inner += '<div class="notify_time"><small>' + timeStr + "</small></div></div>";
...
function markAllAsRead(ids, token, apiUrl) {
  var url = apiUrl.replace(/\/$/, "") + "/notifications/read-mark?token=" + encodeURIComponent(token);
...
function loadInitialNotifications(token, apiUrl) {
  var url = apiUrl + "/notifications/list?token=" + encodeURIComponent(token) + "&limit=" + LIMIT + "&page=" + currentPage;
  var xhr = new XMLHttpRequest();
    ...
    if (xhr.readyState === 4 && xhr.status === 200) {
      try {
        ...
        for (var i = 0; i < notifications.length; i++) {
          ...
          if (item.ViewedAt === null || item.ViewedAt === undefined || item.ViewedAt === 0) {
            newIds.push(item.ID);
          }
        }
