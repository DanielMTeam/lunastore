...
def render_markdown(value):
    ...
    def alert_repl(match):
        ...
        return f'<div class="md-alert md-alert-{alert_type}"><strong>{display_type}</strong><br>{content.strip()}</div>'
    
    text = re.sub(r'^> \[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\r?\n((?:>.*\r?\n?)*)', alert_repl, text, flags=re.MULTILINE)
