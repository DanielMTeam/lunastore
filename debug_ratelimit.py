...
cache.clear()
c = Client()
u, _ = User.objects.get_or_create(username='test_debug', is_superuser=True, is_staff=True)
...
for link in links:
    if 'staticfiles' not in link:
        print("Fetching:", link)
        res = c.get(link)
        print("  -> Status:", res.status_code, "Remaining:", res.get('X-RateLimit-Remaining'))
